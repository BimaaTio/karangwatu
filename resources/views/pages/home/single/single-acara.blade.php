  ...
  <div class="container dark:bg-dark mt-3">
    <div class="flex flex-wrap">
      <div class="flex w-full items-center space-x-4">
        ...
        <h1 class="text-3xl tracking-wide lg:text-3xl dark:text-white mt-5">{{ $data->nama_acara }}</h1>
        <div class="flex-1 border-b border-gray-500 mt-5 dark:border-white"></div>
      </div>
          ...
          <ol class="inline-flex items-center space-x-1 md:space-x-3">
            ...
            <li>
              <div class="flex items-center">
                ...
                <a href="#" class="ml-1 text-sm font-medium text-amber-700 hover:text-amber-600 md:ml-2 dark:text-amber-400 dark:hover:text-white"> {{ $data->nama_acara }} </a>
              </div>
            </li>
          </ol>
        </nav>
        <div class="max-w w-full my-5">
          <img class="h-auto max-w-full" src="{{ asset('storage/'. $data->foto) }}" alt="{{ $data->foto }}">
          <ul class="flex flex-wrap my-4 text-gray-900 dark:text-white">
            ...
            <li class="ml-2">
              <a href="/acara/author/{{ $data->user->name }}" class="text-amber-600 dark:text-amber-500 hover:underline">{{ $data->user->name }}</a>
            </li>
          </ul>
